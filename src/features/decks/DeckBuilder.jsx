...
import { X, PlusCircle, Trash2, BrainCircuit, MinusCircle, CheckCircle, Crown, Wand2 } from 'lucide-react';
import { Modal, Spinner, CardDetailModal } from '../../components/Shared';
import { getGeminiDeckAnalysis, generateDeckWithGemini } from '../../services/api';

// Re-defining CardDisplay here to include deck controls, specific to this view
...
const DeckValidation = ({ deck, tcg }) => {
    ...
    const rules = {
        ...
        commander: [
            ...
            { label: 'Singleton format (1 of each card)', valid: deck.cards.every(c => c.quantity === 1 || c.type_line.includes('Basic Land') || c.oracle_text?.toLowerCase().includes("a deck can have any number of cards named"))},
            { label: 'All cards match commander colors', valid: deck.cards.every(c => c.color_identity.every(color => deck.commander?.color_identity.includes(color))) },
        ]
    };
...
const DeckBuilder = ({ tcg, config, collection, decks, setDecks, activeDeckId, setActiveDeckId, showMessage }) => {
    ...
    const analyzeDeck = async () => {
        ...
        try {
            const result = await getGeminiDeckAnalysis(activeDeck);
            setAnalysis(prev => ({ ...prev, result }));
        } catch (error) {
            console.error("Gemini API Error:", error);
            setAnalysis(prev => ({ ...prev, result: `An error occurred while analyzing the deck: ${error.message}` }));
        } finally {
    ...
    const collectionWithAvailability = useMemo(() => {
        const deckCardCounts = activeDeck?.cards.reduce((acc, card) => {
        ...
        return collection.map(card => ({
            ...card,
            available: card.quantity - (deckCardCounts[card.id] || 0),
            inDeck: deckCardCounts[card.id] || 0
        }));
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 space-y-6">
                 <div className="bg-gray-800/50 p-4 rounded-2xl border border-gray-700 space-y-4 backdrop-blur-sm">
                    <h3 className="font-bold text-lg text-indigo-300">Deck Management</h3>
                    <div className="flex gap-2">
                        <select value={activeDeckId || ''} onChange={e => setActiveDeckId(e.target.value)} className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white">
                            <option value="">-- Select a Deck --</option>
                            {decks.map(deck => <option key={deck.id} value={deck.id}>{deck.name} ({deck.format})</option>)}
                        </select>
                        <button onClick={() => setNewDeckModalOpen(true)} className={`px-4 py-2 text-white font-semibold rounded-lg transition-colors ${config.theme.button}`}><PlusCircle size={20}/></button>
                    </div>
                    <button onClick={() => setAIGenerateModalOpen(true)} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-500 transition-colors text-sm">
                        <Wand2 size={16} /> Generate Deck with AI
                    </button>
                     {activeDeck && (
                        <div className="flex justify-between items-center">
                            <button onClick={analyzeDeck} className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-500 transition-colors text-sm"><BrainCircuit size={16} /> AI Analysis</button>
                            {activeDeck.format === 'commander' &&
                                <button onClick={() => setCommanderModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-500 transition-colors text-sm"><Crown size={16} /> Set Commander</button>
                            }
                 ...
                 {activeDeck && (
                    <div className="bg-gray-800/50 p-4 rounded-2xl border border-gray-700 backdrop-blur-sm">
                        <h3 className="font-bold text-lg text-indigo-300 mb-3">{activeDeck.name} ({activeDeck?.cards.reduce((acc, c) => acc + c.quantity, 0) || 0} cards)</h3>
                        {activeDeck.commander && (
                         ...
                         <div className="max-h-60 overflow-y-auto space-y-2 pr-2">
                            {activeDeck.cards.sort((a,b) => a.name.localeCompare(b.name)).map(card => (
                                <div key={card.id} className="flex justify-between items-center bg-gray-700/50 p-2 rounded-md">
                                    <span className="text-white truncate text-sm cursor-pointer hover:text-indigo-300" onClick={() => setSelectedCard(card)}>
                                        {card.quantity}x {card.name}
                                    </span>
                                    <div className="flex items-center gap-2">
                                         <button onClick={() => updateCardInDeck(card, -1)} className="text-red-500 hover:text-red-400"><MinusCircle size={16}/></button>
                                         <button onClick={() => updateCardInDeck(card, 1)} className="text-green-500 hover:text-green-400"><PlusCircle size={16}/></button>
                                    </div>
                                </div>
                            ))}
                         </div>
            ...
            <div className="lg:col-span-2 bg-gray-800/50 p-4 rounded-2xl border border-gray-700 backdrop-blur-sm">
                <h3 className="font-bold text-lg text-indigo-300 mb-4">Add Cards from Collection</h3>
                {activeDeck ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-4 max-h-[70vh] overflow-y-auto pr-2">
                        {collectionWithAvailability.map(card => (
                ...
                ) : <div className="text-center py-20"><p className="text-gray-400">Please select or create a deck to start adding cards.</p></div>}
            </div>
            ...
            <Modal isOpen={analysis.isOpen} onClose={() => setAnalysis({ isOpen: false, result: '', isAnalyzing: false })} title={`AI Analysis for ${activeDeck?.name}`}>
                {analysis.isAnalyzing ? <Spinner text="The AI is analyzing your deck..." /> : <div className="prose prose-invert prose-p:text-gray-300" dangerouslySetInnerHTML={{ __html: analysis.result }}></div>}
            </Modal>
            <Modal isOpen={confirmDelete.isOpen} onClose={() => setConfirmDelete({ isOpen: false, deckId: null })} title="Confirm Deletion">
                <p className="text-gray-300 mb-6">Are you sure you want to permanently delete this deck?</p>
                <div className="flex justify-end gap-4">
                    <button onClick={() => setConfirmDelete({ isOpen: false, deckId: null })} className="px-6 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-500">Cancel</button>
                    <button onClick={performDelete} className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-500">Delete</button>
                </div>
            </Modal>
        </div>
    );
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Create New Deck">
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label htmlFor="deck-name" className="block text-sm font-medium text-gray-300 mb-1">Deck Name</label>
                    <input type="text" id="deck-name" value={name} onChange={e => setName(e.target.value)} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-lg text-white" required />
                </div>
                <div>
                ...
                <div className="flex justify-end pt-4">
                    <button type="submit" className="px-6 py-2 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-500">Create Deck</button>
                </div>
            </form>
        </Modal>
    );
